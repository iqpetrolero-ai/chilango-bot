...
# Referencias fuertes a background tasks — evita GC silencioso (asyncio docs warning)
...
async def lifespan(app: FastAPI):
    ...
    async def _survey_loop():
        ...
        while True:
            try:
                pedidos = await db.arun(db.get_orders_for_survey, minutes=60)
                for p in pedidos:
                    encuesta = (
    ...
    _spawn_task(_reminder_loop())
    _spawn_task(_survey_loop())
    _spawn_task(_carta_followup_loop())

    yield

    # Shutdown: cancelar tareas en background al recibir SIGTERM
    _tasks_snapshot = list(_background_tasks)
    for t in _tasks_snapshot:
        t.cancel()
    if _tasks_snapshot:
        await _asyncio.gather(*_tasks_snapshot, return_exceptions=True)


app = FastAPI(title="Chilango Bot 🌮", lifespan=lifespan)
...
async def handle_message(phone: str, message: str, phone_number_id: str = None):
    ...
    if phone_clean in delivery_phones:
        ...
        consulta = await db.arun(db.get_pending_delivery_query, phone_clean)
        if consulta:
            ...
            if costo_delivery is not None:
                ...
                _pedidos_confirmados = [
                    p for p in await db.arun(db.get_orders_for_date, _hoy_str)
                    if p.get("phone") == client_phone and p.get("estado") not in ("Cancelado ❌",)
                ]
                if _pedidos_confirmados:
                    await db.arun(db.delete_delivery_query, consulta["id"])
                ...
                for _d in DELIVERIES:
                    _op = await db.arun(db.get_pending_delivery_query, _d["phone"].replace("+",""))
                    if _op and _op.get("client_phone") == client_phone and _op["id"] != consulta["id"]:
                        await db.arun(db.delete_delivery_query, _op["id"])
    ...
    if message.strip() == "1" or any(p in msg_lower for p in PALABRAS_CARTA):
        await db.arun(db.append_message, phone, "user", message)
        respuesta_carta = "📄 [Carta enviada como PDF]" if PDF_URL else "[Carta enviada como texto]"
        await db.arun(db.append_message, phone, "assistant", respuesta_carta)
        if PDF_URL:
    ...
    if not await db.arun(db.is_welcomed, phone):
        await db.arun(db.mark_welcomed, phone)
        if not esta_en_horario():
            reply = mensaje_fuera_horario()
            await send_whatsapp_message(phone, reply, sending_id)
            await db.arun(db.append_message, phone, "user", message)
            await db.arun(db.append_message, phone, "assistant", reply)
        ...
        if msg_lower not in SALUDOS_GENERICOS and message.strip():
            ...
            await db.arun(db.append_message, phone, "user", message)
            await db.arun(db.append_message, phone, "assistant", bienvenida)
        await db.arun(db.mark_unread, phone)
        return
...
async def receive_message(request: Request):
    ...
    if META_APP_SECRET:
        ...
        if not hmac.compare_digest(sig_header, expected):
            print(f"[WEBHOOK] Firma inválida — posible request fraudulento")
    ...
    try:
        ...
        if msg_type == "text":
            ...
            if btn_id == "ver_carta":
                ...
                await db.arun(db.append_message, phone, "user", "Hacer un pedido", ts=now_ts)
                ...
                await db.arun(db.append_message, phone, "user", "Sí, quiero hablar con el equipo", ts=now_ts)
...
async def health():
    try:
        await db.arun(db.health_check)
        db_status = "ok"
    except Exception as e:
        db_status = f"error: {e}"
    return {"status": "ok", "bot": "Chilango 🌮", "db": db_status}
...
async def _notify_order_camino(order: dict):
    ...
    if order.get("id"):
        await db.arun(db.mark_order_camino, order["id"])
...
async def pedidos_panel(
    ...
    if fecha_sel == hoy:
        pedidos = await db.arun(db.get_orders_today)
    else:
        pedidos = await db.arun(db.get_orders_for_date, fecha_sel)
...
async def api_actualizar_estado(
    ...
    order = await db.arun(db.get_order_by_id, order_id)
    await db.arun(db.update_order_estado, order_id, estado)
    if estado == "En camino 🛵":
...
async def api_eliminar_pedido(
    ...
    order_id = int(data.get("order_id", 0))
    if order_id:
        await db.arun(db.delete_order, order_id)
        return JSONResponse({"status": "ok"})
...
async def api_delivery_pendientes(
    ...
    queries = await db.arun(db.get_all_pending_cost_queries)
    result = []
    for q in queries:
        sugg = await db.arun(db.get_delivery_cost_suggestion, q.get("direccion", ""))
        result.append({
...
async def api_enviar_costo_delivery(
    ...
    _pedidos_hoy = [
        p for p in await db.arun(db.get_orders_for_date, _hoy)
        if p.get("phone") == phone and p.get("estado") not in ("Cancelado ❌",)
    ]
    if _pedidos_hoy:
        await db.arun(db.delete_pending_cost_query, phone)
    ...
    all_queries = await db.arun(db.get_all_pending_cost_queries)
    query = next((q for q in all_queries if q["client_phone"] == phone), None)
    if query:
        await db.arun(
...
async def mark_read(phone: str, credentials: HTTPBasicCredentials = Depends(verificar_admin)):
    await db.arun(db.mark_read, phone)
    return JSONResponse({"status": "ok"})
...
async def admin_menu(credentials: HTTPBasicCredentials = Depends(verificar_admin)):
    items = await db.arun(db.get_menu_items)
    grupos: dict = {}
    for it in items:
...
async def api_update_menu_item(
    ...
    if not item_id or not nombre or precio < 0:
        return JSONResponse({"status": "error", "msg": "Datos inválidos"}, status_code=400)
    await db.arun(db.update_menu_item, item_id, nombre=nombre, descripcion=descripcion,
...
async def api_metricas(
    ...
    return JSONResponse(await db.arun(db.get_metricas, dias))
...
async def admin_zonas_delivery(credentials: HTTPBasicCredentials = Depends(verificar_admin)):
    zonas = await db.arun(db.get_delivery_zones_summary)
...
async def imprimir_pedido(
    ...
    import asyncio as _asyncio_print
    order = await db.arun(db.get_order_by_id, order_id)
    if not order:
    ...
    def _get_fecha_hora():
        with db._conn() as c:
            return c.execute("SELECT fecha, hora FROM orders WHERE id=?", (order_id,)).fetchone()

    row = await _asyncio_print.to_thread(_get_fecha_hora)
...
async def delete_conversation(
    ...
    if not phone:
        return JSONResponse({"status": "error"}, status_code=400)
    await db.arun(db.delete_conversation, phone)
...
async def pausar_bot_conv(
    ...
    if not phone:
        return JSONResponse({"status": "error"}, status_code=400)
    await db.arun(db.mark_escalated, phone)
...
async def reactivar_bot_conv(
    ...
    if not phone:
        return JSONResponse({"status": "error"}, status_code=400)
    await db.arun(db.reset_escalation, phone)
...
async def api_toggle_pausa(
    ...
    await db.arun(db.set_config, "bot_pausado", value)
...
async def api_actualizar_puntos(
    ...
    await db.arun(db.update_customer_points, phone, puntos)
    return JSONResponse({"status": "ok"})
